...
// 2. Cargar Datos
try {
    $sql_load = "SELECT nombre_completo, usuario, email, telefono, foto_perfil, genero, firma_imagen_path, fecha_nacimiento FROM usuarios WHERE id = :id";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'actualizar_info') {
        ...
        try {
            $pdo->prepare("UPDATE usuarios SET nombre_completo=?, email=?, telefono=?, genero=?, fecha_nacimiento=? WHERE id=?")
                ->execute([$nombre, $email, $tel, $gen, $fecha_nac, $id_usuario]);
            ...
            if (!empty($firma_b64)) {
                ...
                if (file_put_contents($dir_f . $fname, $decoded)) {
                    ...
                    $pdo->prepare("UPDATE usuarios SET firma_imagen_path=? WHERE id=?")->execute([$fname, $id_usuario]);
...
<div class="container-fluid px-md-4 py-4 mb-5 flex-grow-1" style="max-width: 1600px;">
